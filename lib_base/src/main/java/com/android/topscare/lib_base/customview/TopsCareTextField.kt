...
class TopsCareTextField : LinearLayout {
    var binding: LayoutTopsCareTextFieldBinding
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.layout_tops_care_text_field, this, true)
        bindActions(binding)
    }
    ...
    private fun bindActions(binding: LayoutTopsCareTextFieldBinding) {
...
fun TopsCareTextField.setTextAttrChanged(listener: InverseBindingListener) {
    if (listener == null)
        return
...
fun TopsCareTextField.setOnClickEndDrawable(callback: () -> Unit) {
    onEndDrawableClicked = callback
}

@BindingAdapter("app:text")
fun TopsCareTextField.setText(value: String?) {
...
fun TopsCareTextField.setMaxLength(length: Int) {
    binding.maxLength = length
}

@InverseBindingAdapter(attribute = "app:text")
fun TopsCareTextField.getText(): String {
    return binding.editText.text.toString()
}

@BindingAdapter("app:hint")
fun TopsCareTextField.setHint(value: String?) {
    binding.hint = value ?: ""
}

@BindingAdapter("app:readOnly")
fun TopsCareTextField.setReadOnly(value: Boolean) {
    binding.editText.isEnabled = !value
}

@BindingAdapter("app:uiState")
fun TopsCareTextField.setUiState(state: DataState<Unit>?){
    state?.let {
        binding.uiState = it
    }
}